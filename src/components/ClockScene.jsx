...
/**
 * Drag-to-pour reagent beaker: DragTipper handles the interaction; this
 * wrapper supplies the beaker visuals and stream colour.
 */
function PourableBeaker({ home, target, label, liquidColor, enabled, onPour }) {
  const stream = useMemo(
    ...
    // from the anchor; stream falls from there to just above the target rim
    () => ({ dx: 0.095, topDy: -0.002, bottomDy: 0.1, r1: 0.0032, r2: 0.0022, color: liquidColor, opacity: 0.75, roughness: 0.1 }),
    [liquidColor],
  )
  return (
    <DragTipper home={home} target={target} enabled={enabled} onPour={onPour} stream={stream}>
      <BeakerGlass r={0.032} h={0.09} liquidColor={liquidColor} fill={enabled ? 0.55 : 0.18} />
      <Text
        font={LAB_FONT}
        position={[0, 0.115, 0]}
        fontSize={0.016}
