...
## iter-59 — test mode: no oracle *labels*, no MockPaper reachability (2026-08-14)
Rule: in testMode nothing on screen may name or imply instant verification, and no worked-answer material is reachable. Companion to iter-58's no-verdict rule.
- CheckResult.jsx exports `CheckVerb({ practice })`: renders the practice verb normally, "Record answer" in test mode. Swapped into all 11 submit buttons (Chroma/Electro/Flame/Qual "Check identification", Organic/Peroxide "Check conclusion", Distill "Check technique", Gas/Grav/Solubility "Check", IodineRate "Mark practical evidence").
- MockPaper (worked marking = indirect oracle) gated on `!testMode` at all 4 hosts: TitrationUI mock-open + mock-open-mobile, ClockUI mock-open-clock, EnthalpyUI mock-open-enthalpy (Clock/Enthalpy gained testMode selectors).
- Gate tests/gates/testmode.py extended: flame verb flips Check identification → Record answer → restored; page body free of Check-verbs in test mode; enthalpy completed run offers mock in practice, `[data-testid^="mock-open"]` count 0 in test mode, restored on exit. Solo PASS (~60s), evidence testmode-enthalpy-no-mock.png.
- eslint clean; prepush OK (PRE-PUSH OK, iter59-prepush.log); dist frozen .harness-evidence/iter59-dist.sha256 (46 files, main-CB0LEsXz.js); compressed 1,052,349 B < 1,100,000 budget. Full 37-gate suite: 37/37 green on frozen dist (14 gates run 1 + 23 gates run 2 after a sandbox restart killed run 1 mid-suite at `pour`; dist sha256-verified unchanged between runs, 0 FAIL total).
- NEW (ledger, from iter-59 screenshot audit): enthalpy LIVE CALCULATIONS panel still shows fully worked ΔT/q/n/ΔH formulae in test mode — indirect coaching aid; decide whether to blank values or hide panel in a later iter (same question for any other always-on derived readouts).

## iter-60 — test mode: no derived-answer readouts (2026-08-14)
Rule: in testMode no on-screen readout may display values derived *from the student's results* (the answer the exam asks them to compute). Raw observations stay visible; derived products (ΔT/q/ΔH, 1000/t) and calc walkthroughs do not. Closes iter-59's ledger item.
- EnthalpyUI: "Live Calculations" panel (worked ΔT/q/n/ΔH) wrapped in `!testMode`; "Show Calculations" button likewise gated.
- ClockUI: results-table 1000/t cell renders "—" in test mode (raw time stays); "Show Calculations" gated on `!testMode`.
- TitrationUI: adopted the orphaned CalcSheet properly — practice-only "Show Calculations" button + `!testMode`-gated CalcSheet render; App.jsx dead showTitrationCalc state/render/import deleted.
- Not touched by design: Chroma reference Rf table (data-booklet-style reference values, not derived from student results).
- Gate tests/gates/testmode.py 232→286 lines: enthalpy Live-Calculations present/absent/restored (lower-cased matching — header is CSS-uppercased so innerText reflects text-transform); new clock section runs a real 0.100 M practical (~8 real s at CLOCK_TIME_SCALE=5, paced on clock-time testid with 20 s stall guard), asserts practice row ends in numeric 1000/t, test-mode row ends "—", Show Calculations hidden, both restored on exit; new shot testmode-clock-no-rate.png.
- Gate-dev note: run 1 FAILed on case-sensitive "Live Calculations" match (test-harness string bug, fixed in gate); run 2 killed by sandbox restart mid-log; run 3 clean PASS 103s, all 39 checks, 5 evidence shots reviewed (enthalpy test-mode panel: raw inputs only; clock test-mode row `0.100 | 40.0 | —`).
- eslint clean; prepush OK (PRE-PUSH OK); dist frozen .harness-evidence/iter60-dist.sha256 (46 files, main-CcCXc4xR.js); compressed 1,052,383 B < 1,100,000 budget. Full 37-gate suite on frozen dist: 37/37 green, 0 FAIL (serial ~45 min, log iter60-full.log); dist sha256-verified unchanged after suite.

## iter-61 — bench/room surface texture pass (2026-08-14)
Rule: no large surface may be a single flat colour (bench was flat #31363c, floor #c9d2da, walls flat — AAABench tell #4).
- NEW src/components/scene/surfaces.js: seeded mulberry32 PRNG + procedural CanvasTexture makers — makeEpoxyTexture (mottle/speckle/scratches/ghost rings), makeEpoxyRoughness (linear-space sheen map), makeVinylFloorTexture (terrazzo flecks + traffic wear), makePlasterTexture (blotch + roller streaks). ClampToEdge single-cover (no tiling), anisotropy 8, deterministic. Optional statKey records {w,h,mean,std} sampled luminance to window.__labSurfaceStats for gate assertions.
- LabRoom.jsx: bench epoxy seed 7 + roughnessMap seed 8 (roughness=1); floor vinyl seed 21; back wall plaster seed 40 (#e3e9f0), sides 41/42 (#dde4ec), front 43 (#e0e6ee). props.jsx BackCounter top epoxy seed 91 (#2b2e33, 1024x256).
- Tuning: first pass invisible (dark base + glancing-angle mip blur) — amplitudes needed ~3x what looked right in the canvas (mottle 0.1→0.42, speckle 0.16→0.45); roughness map added for sheen variation. LOOK verdict: bench reads as worn epoxy; walls calm under fog (intended).
- Gate tests/gates/gfx.py 213→232 lines: asserts __labSurfaceStats std thresholds bench ≥5 / floor ≥4 / backWall ≥2 / counter ≥4, bench mean in (0,110). Shipped values: bench std 11.93 mean 59.97, floor 9.26, backWall 5.13, counter 9.63. Screenshot high-pass pixel metrics proved too weak for dark glancing surfaces (0.063 vs 0.105) — source-level stats hook instead.
- eslint clean; prepush OK (PRE-PUSH OK); dist frozen .harness-evidence/iter61-dist.sha256 (46 files, main-BtfuOALL.js); compressed 1,053,739 B < 1,100,000 budget.
- Full 37-gate suite on frozen dist: 37/37 green total. Leg 1: 19 PASS (classroom→cooling) before a sandbox restart killed the run; dist sha256-verified unchanged. Leg 2: 16/18 (read + mock FAILed on cold-start Playwright 30s timeouts — sandbox has nproc=1). Solo re-run same dist: read PASS 82s, mock PASS 61s → timing flakes, not regressions. Dist sha256-verified intact after all runs. Evidence shots LOOK-reviewed (gfx-ultra, flame, distill, landing).
- Env note: this sandbox has 1 CPU; long gates can flake on 30s selector timeouts under cold start. Rerun flaky gates solo before treating as regressions.

## iter-62 — asserted state must be visible in the scene: grav Bunsen flame (2026-08-14)
Rule: when the UI asserts a physical state ("Heating strongly over the Bunsen…"), the 3D scene must visibly show it — a stranger reading the caption and seeing a cold burner calls the sim fake in 3 s (AAABench). Instance fixed: grav; gas "Reacting…" bubble stream is the next ledger candidate.
- NEW src/components/scene/flameShell.js: shared flameGeometry(rBase,rBulge,h) lathe teardrop + flameAlphaMap() + BUNSEN_BLUE_OUTER/INNER, extracted from FlameScene's iter-56 flame. Companion rule: no bare <coneGeometry> flames anywhere — teardrop lathe + alpha map only.
- FlameScene.jsx refactored to import from flameShell (behaviour identical; flame gate solo PASS 68s pre-suite).
- GravScene.jsx: Bunsen gets outer flameGeometry(0.008,0.0165,0.06) rooted y=0.07 (tip licks crucible at 0.127) + inner core (0.0065,0.0095,0.032), opacity .85/.8, alphaMap, depthWrite false, triple-sine flicker; flame pointLight intensity 0.5+sin(11t)*0.11+sin(23t)*0.05 dist 0.38 (first cut 0.9/0.5 washed the bench nightclub-blue — tuned by LOOK); crucible emissive ramps to 0.9 heating / 0.2 cooling with warm #ff7a3a pointLight tied to emissive.
- Measured (LOOK probe, idle vs heating): burner-box (430,200,720,600 @1280x720) mean diff 25.2, full-canvas 12.5, idle noise ~0.
- Gate tests/gates/grav.py 205 lines: captures idle screenshot pre-heat, then at cycle 0 does fast DOM reads FIRST (phase attr, weigh disabled) before the slow SwiftShader screenshot — screenshot-first ordering blew past the 2.5 s HEAT_MS window and failed 2 checks (gate-timing lesson: DOM reads before slow captures inside timed sim windows). Asserts burner-box mean diff > 6 (measured 20.2 passing run). Solo PASS 94s.
- eslint clean; prepush OK (PRE-PUSH OK); dist frozen .harness-evidence/iter62-dist.sha256 (46 files, main-BTflC3sF.js); compressed 1,053,829 B < 1,100,000 budget.
- Full 37-gate suite on frozen dist: 37/37 green, 0 FAIL. Leg 1: 26 PASS (classroom→grav) before a sandbox restart killed the run; leg 2: remaining 11/11 PASS (gfx→testmode). Dist sha256-verified unchanged across legs and after a post-suite prepush rebuild (build reproducible).

## opt-A — asset weight: landing PNGs → WebP, og.png quantized (2026-08-14)
Track pivot: user directive 2026-08-14 17:24 — strictly optimisation research + implementation. Phase A = shipped-byte weight; measured dist gzip 1,053,829 B → 786,539 B (−267,290 B, −25.4%) against the 1,100,000 budget.
- ExperimentMenu.jsx: the two landing <img> srcs → /media/lab-kinetics.webp + /media/lab-mobile.webp (width/height attrs kept; landing gate asserts naturalWidth 1280×720 — WebP preserves intrinsic dims).
- NEW public/media/lab-kinetics.webp 39,216 B + lab-mobile.webp 17,808 B (PIL WEBP q=82 method=6); the two PNGs (≈180 KB + ≈80 KB) git rm'd. Landing desktop+mobile LOOK-reviewed vs before: pixel-identical layout, crisp.
- public/og.png re-emitted quantized (256-colour MEDIANCUT + FLOYDSTEINBERG dither, optimize) → 68,961 B, still 1200×630 (gate checks size/dims only; visually crisp on LOOK). tools/generate_social_assets.py updated: CAPTURE reads the webp, og output quantized — regeneration stays consistent.
- vite.config.js workbox: globPatterns += webp; globIgnores og.png (social crawlers fetch it server-side; precaching it wastes 69 KB of every client's cache). sw.js precache 42 entries, og.png confirmed absent.
- eslint clean; prepush OK (PRE-PUSH OK); dist gzip 786,539 B < 1,100,000 budget; dist sha256-verified byte-identical across gate runs AND after post-suite prepush rebuild (46-file manifest).
- Full 37-gate suite on frozen dist: 37/37 green, 0 FAIL. Leg 1: 34 PASS before a sandbox restart killed it mid-peroxide; leg 2: iodine_rate PASS 117s + testmode PASS 119s; peroxide FAILed twice at 22s cold-start ("first automatic 20 s reading") then full PASS 200s solo (30/30 asserts) — flake, not regression: same assert passed on this exact dist in leg 1.
- Flake root cause READ (not yet fixed, candidate next iter): PeroxideUI.jsx line 127 displays Math.round(timeSec) → shows "20 s" at 19.5, but the 20 s reading records in the store tick at ≥ 20.0; gate's wait_for_function fires on the displayed "20" then asserts reading-count 2/… → 0.5-sim-sec race, exposed on cold 1-CPU starts. Correct fix is display/recording consistency (e.g. Math.floor in UI), never loosening the gate.
- Phase B research (not started): LabViewport chunk 319,398 gz (three+drei+troika+all scenes; troika pulled by drei Text, 27 refs; RGBE/EXRLoader via drei Environment). manualChunks deliberately keeps three lazy — splitting hoisted bindings put 304 kB gz on landing. Fonts inter-var 48K / jbmono 31K / chemlab-mono 30K all precached; main 64,249 gz; vendor-react 59,879 gz.

## opt-B: label font subset regenerated (tofu fix) + gainmap-js stubbed out (-27.6 KB gz)
- Bundle optimisation phase B, on top of opt-A (f124880).
- Change 1: public/fonts/chemlab-mono.woff regenerated by NEW tools/subset_label_font.py.
  Old 124-char subset (30,552 B) was missing U+00B7 MIDDLE DOT and U+2191 UP ARROW used by
  IodineRateScene ('CO2 ^ . ACID QUENCHED') and PeroxideScene ('C . ...') 3D labels -> live
  tofu boxes. New generator downloads upstream JetBrainsMono-Regular.ttf, scans src/ for all
  non-ASCII chars, subsets printable ASCII + found chars (138), --layout-features= --no-hinting
  --flavor=woff --with-zopfli. New woff 7,772 B (-75%), 147 glyphs, cmap covers B7/2191/B0/2082/B3
  (VERIFIED via fontTools). Cache files gitignored.
- Change 2: NEW src/lib/gainmapStub.js + vite.config.js resolve.alias maps '@monogrid/gainmap-js'
  to throwing stub. Dead code: Environment uses procedural Lightformers only, no jpg/webp env maps
  (LabViewport.jsx). Removes 15,428 B raw from LabViewport chunk.
- Dist gzip total 786,539 -> 758,954 B (baseline before opt-A: 1,053,829; cumulative -28%).
- 37/37 gates green on frozen dist (sha256 manifest verified unchanged pre/post prepush).
