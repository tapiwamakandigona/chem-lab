...
Flow: menu shows course-open with 0/17 → panel lists 17 units all not-done →
...
badge 2/17 → reload page → progress persists from localStorage.
...
with sync_playwright() as p:
    ...
    check("badge starts 0/17", "0/17" in pg.locator('[data-testid="course-open"]').inner_text())

    pg.locator('[data-testid="course-open"]').click()
    time.sleep(0.4)
    check("panel opens", pg.locator('[data-testid="course-panel"]').count() == 1)
    check("17 units listed", pg.locator('[data-testid^="course-unit-"]').count() == 17)
    ...
    check("progress reads 0/17", prog.get_attribute("data-done") == "0" and prog.get_attribute("data-total") == "17")
    ...
    time.sleep(0.8)
    check("badge now 2/17", "2/17" in pg.locator('[data-testid="course-open"]').inner_text(),
    ...
    time.sleep(2)
    check("after reload badge still 2/17",
          "2/17" in pg.locator('[data-testid="course-open"]').inner_text())
    pg.locator('[data-testid="course-open"]').click()
    time.sleep(0.4)
